...
_DEAD_CODE_ALLOWLIST: dict[tuple[str, str], str] = {
    ...
    ("tracker.py", "get_regional_recent_bias"): (
        ...
        "2026-08-22: briefly wired into weather_markets._get_combined_station_"
        "bias() as a capped lean, then reverted -- an opus review caught a real "
        "contamination path (a correlated city with a thin/unverified static-"
        "bias entry, e.g. Miami, could leak its own PERSISTENT residual into a "
        "neighbour, e.g. Atlanta, disguised as a transient regime signal). "
        "Fixed at the source (this function now excludes any correlated city "
        "whose own dynamic station-bias hasn't cleared the same count>=10 "
        "maturity floor _get_combined_station_bias uses) -- a real, kept "
        "correctness improvement independent of wiring. But re-running the "
        "retrospective validation against the FIXED version showed the "
        "original r=0.38 signal was itself substantially an artifact of that "
        "same contamination: post-fix, r=0.08 (n=35, was 109), sign agreement "
