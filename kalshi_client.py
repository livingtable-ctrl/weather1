...
class KalshiClient:
    ...
    def place_order(
        ...
        # the same client_order_id is resubmitted. AUD batch-23 #1:
        # time_in_force is part of the key (not just an inert field on the
        # request body) because two calls with everything else equal --
        # notably ticker+side+action+count+cycle -- but different
        # time_in_force are NOT the same order attempt: a GTC entry and a
        # later IOC taker-cross replacement of it (order_executor.
        # _replace_live_order) can otherwise round to the identical price
        # and silently dedupe against each other, which would make the
        # taker-cross a no-op that logs success while the position never
        # re-enters. Callers that need each real attempt (a reprice, an
        # exit retry) to get its own key regardless of price/time_in_force
        # repetition fold a per-attempt discriminator into the `cycle`
        # string they pass in, rather than this method dedup'ing across
        # attempts it can't distinguish (see order_executor._replace_live_order/
        # _amend_live_order/_exit_live_position).
        idempotency_input = (
            f"{ticker}:{side}:{action}:{count:.2f}:{price:.4f}:{time_in_force}:"
            f"{cycle or uuid.uuid4()}"
        )
