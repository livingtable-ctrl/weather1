...
def main() -> int:
    ...
    print("     DO THIS BEFORE RUNNING ANYTHING FROM THE CLONE. A fresh clone")
    print("     now brings no data/ files at all (batch-79 untracked the five")
    print("     calibration files; seeds/ holds the first-run copies). But the")
    print("     first process that imports paths.py materializes those seeds")
    print("     into data/ -- so if you run even a smoke test first, this copy")
    print("     turns into an overwrite prompt, and declining it silently keeps")
    print("     the seed and discards this bundle's learned state. If you have")
    print("     already run something, answer 'yes to all' when it prompts.")
    print("     (Before batch-79 the clone itself carried the five files, so")
    print("     the wrong order discarded learned state with no prompt at all.)")
