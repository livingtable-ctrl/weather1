---
source_file: "tests/test_debug_fixes.py"
type: "code"
community: "Community 78"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_78
---

# test_debug_fixes.py

## Connections
- [[Regression tests for the full-program debug session fixes. Covers A —…]] - `rationale_for` [EXTRACTED]
- [[TestAnalysisAttemptsUpsert]] - `contains` [EXTRACTED]
- [[TestEntryProbFalsyZero]] - `contains` [EXTRACTED]
- [[TestEnvVarFallback]] - `contains` [EXTRACTED]
- [[TestLogPredictionWarning]] - `contains` [EXTRACTED]
- [[TestPlacePaperOrderValidation]] - `contains` [EXTRACTED]
- [[TestSyncOutcomesWarning]] - `contains` [EXTRACTED]
- [[_env_float()_1]] - `calls` [EXTRACTED]
- [[_env_int()_1]] - `calls` [EXTRACTED]
- [[batch_log_analysis_attempts()]] - `calls` [EXTRACTED]
- [[covariance_kelly_scale()]] - `calls` [EXTRACTED]
- [[get_attribution()]] - `calls` [EXTRACTED]
- [[log_analysis_attempt()]] - `calls` [EXTRACTED]
- [[log_prediction()]] - `calls` [EXTRACTED]
- [[place_paper_order()_1]] - `calls` [EXTRACTED]
- [[sync_outcomes()]] - `calls` [EXTRACTED]
- [[tmp_paper()]] - `contains` [EXTRACTED]
- [[tmp_tracker()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_78