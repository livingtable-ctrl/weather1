---
source_file: "tests/test_hurricane_climatology.py"
type: "code"
community: "Community 98"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_98
---

# test_hurricane_climatology.py

## Connections
- [[TestBootstrapCi]] - `contains` [EXTRACTED]
- [[TestBootstrapCiNextEvent]] - `contains` [EXTRACTED]
- [[TestCountAsOfDay]] - `contains` [EXTRACTED]
- [[TestExceedanceProbability]] - `contains` [EXTRACTED]
- [[TestFetchHurdat2RawCacheWrite]] - `contains` [EXTRACTED]
- [[TestFirstHurricanePosition]] - `contains` [EXTRACTED]
- [[TestFirstHurricanePositionOutcomes]] - `contains` [EXTRACTED]
- [[TestFirstOccurrenceDay]] - `contains` [EXTRACTED]
- [[TestNextEventOutcomes]] - `contains` [EXTRACTED]
- [[TestNextEventProbability]] - `contains` [EXTRACTED]
- [[TestParseHurdat2]] - `contains` [EXTRACTED]
- [[TestSeasonEndCounts]] - `contains` [EXTRACTED]
- [[TestSeasonEndTotalDistribution]] - `contains` [EXTRACTED]
- [[Tests for hurricane_climatology.py -- HURDAT2 parsing, season-count…]] - `rationale_for` [EXTRACTED]
- [[_fixture_storms()]] - `contains` [EXTRACTED]
- [[_next_event_fixture_storms()]] - `contains` [EXTRACTED]
- [[_storm_order_fixture_storms()]] - `contains` [EXTRACTED]
- [[backtest.py_2]] - `cites` [EXTRACTED]
- [[fetch_hurdat2_raw()]] - `references` [EXTRACTED]
- [[first_hurricane_position()]] - `references` [EXTRACTED]
- [[hurricane_climatology.py]] - `imports` [EXTRACTED]
- [[next_event_outcomes()]] - `references` [EXTRACTED]
- [[parse_hurdat2()]] - `references` [EXTRACTED]
- [[season_end_total_distribution()]] - `references` [EXTRACTED]
- [[test_hurricane_markets.py]] - `cites` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_98