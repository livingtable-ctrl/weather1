---
type: community
cohesion: 0.11
members: 36
---

# Community 59

**Cohesion:** 0.11 - loosely connected
**Members:** 36 nodes

## Members
- [[39 Bayesian Kelly — integrate kelly_fraction over a uniform posterior on…]] - rationale - weather_markets.py
- [[Accuracy Improvement Plan]] - document - docs/plans/2026-04-19-accuracy-improvements.md
- [[Accuracy Improvement Plan v2]] - document - docs/plans/2026-04-19-accuracy-improvements-v2.md
- [[Execution Pipeline Plan]] - document - docs/superpowers/plans/2026-04-10-execution-pipeline.md
- [[Grade Audit Final Report]] - document - docs/grade_audit/report.md
- [[Grade Audit Preamble]] - document - docs/grade_audit/preamble.md
- [[Grade Audit Synthesis Prompt]] - document - docs/grade_audit/synthesis.md
- [[Kelly sizing block (analyze_trade)]] - code - weather_markets.py
- [[Phase 2 Infrastructure Plan]] - document - docs/plans/2026-04-10-phase2-infrastructure.md
- [[Phase 7 Data Safety Plan]] - document - docs/plans/2026-04-10-phase7-data-safety.md
- [[Safe IO Grade Audit]] - document - docs/grade_audit/outputs/safe_io.py.md
- [[Schema Validator Grade Audit]] - document - docs/grade_audit/outputs/schema_validator.py.md
- [[Score Improvement Plan]] - document - docs/plans/2026-04-17-score-improvements.md
- [[Weather Markets Grade Audit]] - document - docs/grade_audit/outputs/weather_markets.py.md
- [[_current_forecast_cycle()_1]] - code - weather_markets.py
- [[bayesian_kelly()]] - code - weather_markets.py
- [[config.py_1]] - code - config.py
- [[consistency.py_1]] - code - consistency.py
- [[cron.py_2]] - code - cron.py
- [[execution_log.py_2]] - code - execution_log.py
- [[feature_importance.py_1]] - code - feature_importance.py
- [[kalshi_client.py_2]] - code - kalshi_client.py
- [[kalshi_ws.py_1]] - code - kalshi_ws.py
- [[main.py_1]] - code - main.py
- [[metar.py_1]] - code - metar.py
- [[ml_bias.py_2]] - code - ml_bias.py
- [[monte_carlo.py_2]] - code - monte_carlo.py
- [[notify.py_1]] - code - notify.py
- [[nws.py_1]] - code - nws.py
- [[output_formatters.py_1]] - code - output_formatters.py
- [[paper.py_1]] - code - paper.py
- [[param_sweep.py_1]] - code - param_sweep.py
- [[regime.py_2]] - code - regime.py
- [[safe_io.py_2]] - code - safe_io.py
- [[schema_validator.py_1]] - code - schema_validator.py
- [[weather_markets.py_2]] - code - weather_markets.py

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Community_59
SORT file.name ASC
```

## Connections to other communities
- 20 edges to [[_COMMUNITY_Community 36]]
- 5 edges to [[_COMMUNITY_NWSCircuit-Breaker Data Validation]]
- 4 edges to [[_COMMUNITY_Kelly Sizing Property-Based Tests]]
- 4 edges to [[_COMMUNITY_ML Bias Correction & Audit Plans]]
- 4 edges to [[_COMMUNITY_Community 47]]
- 4 edges to [[_COMMUNITY_Backtest Engine & Atomic Writes]]
- 3 edges to [[_COMMUNITY_Community 160]]
- 3 edges to [[_COMMUNITY_Black Swan Detection & Walk-Forward Backtest]]
- 3 edges to [[_COMMUNITY_Ensemble Weight Blending Tests]]
- 3 edges to [[_COMMUNITY_Community 118]]
- 3 edges to [[_COMMUNITY_Community 33]]
- 2 edges to [[_COMMUNITY_METAR Settlement Monitoring]]
- 2 edges to [[_COMMUNITY_Forecasting Persistence Model Tests]]
- 2 edges to [[_COMMUNITY_Community 86]]
- 2 edges to [[_COMMUNITY_Community 326]]
- 2 edges to [[_COMMUNITY_Climatology & Climate Index Fetching]]
- 1 edge to [[_COMMUNITY_Community 58]]
- 1 edge to [[_COMMUNITY_Community 71]]
- 1 edge to [[_COMMUNITY_Community 94]]
- 1 edge to [[_COMMUNITY_Community 296]]

## Top bridge nodes
- [[Grade Audit Final Report]] - degree 33, connects to 12 communities
- [[weather_markets.py_2]] - degree 19, connects to 8 communities
- [[Phase 2 Infrastructure Plan]] - degree 14, connects to 4 communities
- [[Accuracy Improvement Plan]] - degree 13, connects to 4 communities
- [[Weather Markets Grade Audit]] - degree 10, connects to 4 communities