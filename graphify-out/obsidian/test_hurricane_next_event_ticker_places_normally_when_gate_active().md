---
source_file: "tests/test_shadow_predictions.py"
type: "code"
community: "Community 704"
location: "L636"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_704
---

# test_hurricane_next_event_ticker_places_normally_when_gate_active()

## Connections
- [[Once _hurricane_next_event_gates_active() is True, a next-event opp places…]] - `rationale_for` [EXTRACTED]
- [[_auto_place_trades()]] - `calls` [EXTRACTED]
- [[_fetch()]] - `calls` [EXTRACTED]
- [[_make_flat_opp()]] - `calls` [EXTRACTED]
- [[_place_everything_setup()]] - `calls` [EXTRACTED]
- [[test_shadow_predictions.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_704