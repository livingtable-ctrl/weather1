---
type: community
cohesion: 0.10
members: 30
---

# Community 83

**Cohesion:** 0.10 - loosely connected
**Members:** 30 nodes

## Members
- [[dot-test_ci_reuses_bootstrap_ci_next_event_directly()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_conditional_mode_excludes_years_already_resolved()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_custom_kt_threshold_major_hurricane()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_hurricane_attainment_race_not_naming_race()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_named_storms_in_naming_order_excludes_unnamed_systems()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_no_qualifying_storm_returns_none()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_pass_zero_is_the_documented_unconditional_identity()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_position_is_first_storm_to_reach_kt_not_first_storm_of_season()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_probability_reuses_next_event_probability_directly()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_same_day_tie_breaks_to_lowest_sequence_number()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_single_storm_season()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_unconditional_mode_hand_computed()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_unnamed_storm_does_not_consume_a_name_index_slot()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_untracked_kt_raises_valueerror()]] - code - tests/test_hurricane_climatology.py
- [[dot-test_zero_storm_season_contributes_a_real_false_not_dropped()]] - code - tests/test_hurricane_climatology.py
- [[2001's AL01 never reaches hurricane strength -- AL02 (the 2nd named storm) is…]] - rationale - tests/test_hurricane_climatology.py
- [[2002 (no storm ever reaches HU) must still contribute a real False for any real…]] - rationale - tests/test_hurricane_climatology.py
- [[2004 AL01 formed first (named first) but reaches HU on Sep 10; AL02 formed…]] - rationale - tests/test_hurricane_climatology.py
- [[Covers both placeholder shapes _UNNAMED_STORM_NAMES filters -- the literal…]] - rationale - tests/test_hurricane_climatology.py
- [[Opus-review-caught (2026-08-07, CRITICAL) 2005's UNNAMED system (AL02) has a…]] - rationale - tests/test_hurricane_climatology.py
- [[Opus-review-caught (2026-08-07, LOW) the fixture only produces 5 outcomes,…]] - rationale - tests/test_hurricane_climatology.py
- [[Same guard first_occurrence_day's own docstring explains an untracked kt must…]] - rationale - tests/test_hurricane_climatology.py
- [[TestFirstHurricanePosition]] - code - tests/test_hurricane_climatology.py
- [[TestFirstHurricanePositionOutcomes]] - code - tests/test_hurricane_climatology.py
- [[_storm_order_fixture_storms()]] - code - tests/test_hurricane_climatology.py
- [[first_hurricane_position_outcomes returns a plain listbool --…]] - rationale - tests/test_hurricane_climatology.py
- [[kt=96 (major hurricane) only AL03 reaches in 2001 -- position=3, different from…]] - rationale - tests/test_hurricane_climatology.py
- [[storms_named_so_far=0 -- every window year eligible. Hand- verified per-year…]] - rationale - tests/test_hurricane_climatology.py
- [[storms_named_so_far=0 must be equivalent to no conditioning at all -- every…]] - rationale - tests/test_hurricane_climatology.py
- [[storms_named_so_far=2 -- eligible years are those whose real position is None…]] - rationale - tests/test_hurricane_climatology.py

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Community_83
SORT file.name ASC
```

## Connections to other communities
- 4 edges to [[_COMMUNITY_Community 98]]

## Top bridge nodes
- [[_storm_order_fixture_storms()]] - degree 17, connects to 1 community
- [[TestFirstHurricanePosition]] - degree 10, connects to 1 community
- [[TestFirstHurricanePositionOutcomes]] - degree 7, connects to 1 community