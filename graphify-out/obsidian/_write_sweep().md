---
source_file: "tests/test_param_sweep_load.py"
type: "code"
community: "Community 190"
location: "L12"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_190
---

# _write_sweep()

## Connections
- [[dot-test_custom_min_trades_respected()]] - `calls` [EXTRACTED]
- [[dot-test_min_trades_floor_filters_low_sample_entries()]] - `calls` [EXTRACTED]
- [[dot-test_none_win_rate_entries_skipped()]] - `calls` [EXTRACTED]
- [[dot-test_out_of_range_value_returns_none()]] - `calls` [EXTRACTED]
- [[dot-test_returns_best_win_rate_among_qualifying()]] - `calls` [EXTRACTED]
- [[dot-test_returns_none_for_empty_results()]] - `calls` [EXTRACTED]
- [[dot-test_returns_none_when_no_results_qualify()]] - `calls` [EXTRACTED]
- [[Path_7]] - `references` [EXTRACTED]
- [[test_param_sweep_load.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_190