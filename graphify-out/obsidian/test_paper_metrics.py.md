---
source_file: "tests/test_paper_metrics.py"
type: "code"
community: "Community 235"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_235
---

# test_paper_metrics.py

## Connections
- [[Grade Audit Module Doc paper.py]] - `references` [EXTRACTED]
- [[TestBreakEvenStop]] - `contains` [EXTRACTED]
- [[TestProfitFactor]] - `contains` [EXTRACTED]
- [[TestSpreadKellyMultiplier]] - `contains` [EXTRACTED]
- [[Tests for profit factor, break-even stop, and spread Kelly multiplier.]] - `rationale_for` [EXTRACTED]
- [[_score_ensemble_members()]] - `calls` [EXTRACTED]
- [[get_profit_factor()]] - `calls` [EXTRACTED]
- [[paper.check_breakeven_stops]] - `calls` [EXTRACTED]
- [[place_paper_order()_1]] - `calls` [EXTRACTED]
- [[spread_kelly_multiplier()]] - `calls` [EXTRACTED]
- [[test_place_paper_order_defaults_model_forecast_means_to_empty_dict()]] - `contains` [EXTRACTED]
- [[test_place_paper_order_stores_model_forecast_means()]] - `contains` [EXTRACTED]
- [[test_place_paper_order_stores_var_on_trade()]] - `contains` [EXTRACTED]
- [[test_score_ensemble_members_falls_back_for_legacy_trade_without_var()]] - `contains` [EXTRACTED]
- [[test_score_ensemble_members_logs_every_model_in_the_generic_dict()]] - `contains` [EXTRACTED]
- [[test_score_ensemble_members_prefers_stored_var()]] - `contains` [EXTRACTED]
- [[test_score_ensemble_members_skips_absent_models()]] - `contains` [EXTRACTED]
- [[test_score_ensemble_members_uses_db_settled_temp()]] - `contains` [EXTRACTED]
- [[update_peak_profits()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_235