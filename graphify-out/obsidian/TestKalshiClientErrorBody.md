---
source_file: "tests/test_phase2_batch_l.py"
type: "code"
community: "Community 283"
location: "L267"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_283
---

# TestKalshiClientErrorBody

## Connections
- [[dot-_make_client()_9]] - `method` [EXTRACTED]
- [[dot-test_check_error_body_passes_on_clean_response()]] - `method` [EXTRACTED]
- [[dot-test_check_error_body_passes_on_non_dict()]] - `method` [EXTRACTED]
- [[dot-test_check_error_body_raises_on_error_field()]] - `method` [EXTRACTED]
- [[dot-test_get_raises_on_error_body()]] - `method` [EXTRACTED]
- [[dot-test_post_raises_on_error_body()]] - `method` [EXTRACTED]
- [[dot-test_post_succeeds_on_clean_response()]] - `method` [EXTRACTED]
- [[KalshiClient]] - `uses` [INFERRED]
- [[Phase 2 Batch L Regression Tests]] - `contains` [EXTRACTED]
- [[_get_post_delete must raise ValueError on 200-with-error responses.]] - `rationale_for` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_283