---
type: community
cohesion: 0.12
members: 25
---

# Community 109

**Cohesion:** 0.12 - loosely connected
**Members:** 25 nodes

## Members
- [[Atomic Write Pattern (os.replace)]] - document - docs/grade_audit/modules/safe_io.md
- [[Daily Loss Check Dead Letter (int timestamp bug)]] - document - docs/grade_audit/outputs/alerts.py.md
- [[Grade Audit Module Doc calibration.py]] - document - docs/grade_audit/modules/calibration.md
- [[Grade Audit Module Doc cron.py]] - document - docs/grade_audit/modules/cron.md
- [[Grade Audit Module Doc safe_io.py]] - document - docs/grade_audit/modules/safe_io.md
- [[Grade Audit Output ab_test.py]] - document - docs/grade_audit/outputs/ab_test.py.md
- [[Grade Audit Output alerts.py]] - document - docs/grade_audit/outputs/alerts.py.md
- [[Grade Audit Output calibration.py]] - document - docs/grade_audit/outputs/calibration.py.md
- [[Grade Audit Output climatology.py]] - document - docs/grade_audit/outputs/climatology.py.md
- [[Grade Audit Output cloud_backup.py]] - document - docs/grade_audit/outputs/cloud_backup.py.md
- [[Implicit brier_score min_days_out Default Gap]] - document - docs/grade_audit/outputs/alerts.py.md
- [[Multi-day days_out Filter Invariant]] - document - docs/grade_audit/modules/tracker.md
- [[Per-Order Kill Switch Check]] - document - docs/grade_audit/modules/cron.md
- [[Three-Layer Defense (Condition Weight Auto-Calibration)]] - document - docs/grade_audit/modules/calibration.md
- [[_find_google_drive() RF1 Silent Registry Failure]] - document - docs/grade_audit/outputs/cloud_backup.py.md
- [[ab_test.py_1]] - code - ab_test.py
- [[alerts.py_1]] - code - alerts.py
- [[calibrate_and_save() RF1 Silent Preservation Failure]] - document - docs/grade_audit/outputs/calibration.py.md
- [[calibration.py_1]] - code - calibration.py
- [[check_alerts() RF1 Silent Exception Swallow]] - document - docs/grade_audit/outputs/alerts.py.md
- [[climatology.py_1]] - code - climatology.py
- [[cloud_backup.py_1]] - code - cloud_backup.py
- [[fetch_historical() RF1 Silent API-Failure Swallow]] - document - docs/grade_audit/outputs/climatology.py.md
- [[get_active_variant() RF1 Silent Exception Swallow]] - document - docs/grade_audit/outputs/ab_test.py.md
- [[test_safe_io.py_1]] - code - tests/test_safe_io.py

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Community_109
SORT file.name ASC
```

## Connections to other communities
- 3 edges to [[_COMMUNITY_Community 32]]
- 2 edges to [[_COMMUNITY_Black Swan Detection & Walk-Forward Backtest]]
- 2 edges to [[_COMMUNITY_Backtest Engine & Atomic Writes]]
- 2 edges to [[_COMMUNITY_Community 103]]
- 1 edge to [[_COMMUNITY_Climatology & Climate Index Fetching]]
- 1 edge to [[_COMMUNITY_Community 194]]
- 1 edge to [[_COMMUNITY_Community 302]]
- 1 edge to [[_COMMUNITY_Community 132]]
- 1 edge to [[_COMMUNITY_Cron Integration Tests]]
- 1 edge to [[_COMMUNITY_Community 121]]
- 1 edge to [[_COMMUNITY_Community 253]]
- 1 edge to [[_COMMUNITY_Community 40]]
- 1 edge to [[_COMMUNITY_Community 451]]
- 1 edge to [[_COMMUNITY_Community 120]]
- 1 edge to [[_COMMUNITY_Tracker SQLite Storage Tests]]
- 1 edge to [[_COMMUNITY_Community 26]]
- 1 edge to [[_COMMUNITY_Community 293]]

## Top bridge nodes
- [[Grade Audit Module Doc cron.py]] - degree 8, connects to 5 communities
- [[alerts.py_1]] - degree 9, connects to 4 communities
- [[Multi-day days_out Filter Invariant]] - degree 6, connects to 3 communities
- [[climatology.py_1]] - degree 5, connects to 3 communities
- [[cloud_backup.py_1]] - degree 4, connects to 2 communities