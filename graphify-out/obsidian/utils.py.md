---
source_file: "utils.py"
type: "code"
community: "NWS/Circuit-Breaker Data Validation"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/NWS/Circuit-Breaker_Data_Validation
---

# utils.py

## Connections
- [[RiskTab.jsx (V3)]] - `conceptually_related_to` [INFERRED]
- [[Shared utilities used across the Kalshi weather trading modules.]] - `rationale_for` [EXTRACTED]
- [[_hash_fingerprint()]] - `contains` [EXTRACTED]
- [[_paper_min_edge_default()]] - `imports` [EXTRACTED]
- [[_setup_logging()_1]] - `contains` [EXTRACTED]
- [[backtest.py]] - `imports_from` [EXTRACTED]
- [[check_config_integrity()]] - `contains` [EXTRACTED]
- [[classify_confidence_tier()]] - `contains` [EXTRACTED]
- [[climatology.py]] - `imports_from` [EXTRACTED]
- [[coalesce_market_price()]] - `contains` [EXTRACTED]
- [[config.py]] - `imports_from` [EXTRACTED]
- [[cron.py]] - `imports_from` [EXTRACTED]
- [[execution_log.py]] - `imports_from` [EXTRACTED]
- [[get_config_fingerprint()]] - `contains` [EXTRACTED]
- [[get_min_edge_for_confidence()]] - `contains` [EXTRACTED]
- [[get_paper_min_edge()]] - `contains` [EXTRACTED]
- [[is_trading_paused()]] - `contains` [EXTRACTED]
- [[main.py]] - `imports_from` [EXTRACTED]
- [[min_prob_edge_for_days_out()]] - `contains` [EXTRACTED]
- [[monte_carlo.py]] - `imports_from` [EXTRACTED]
- [[mos.py]] - `imports_from` [EXTRACTED]
- [[normal_cdf()]] - `contains` [EXTRACTED]
- [[nws.py]] - `imports_from` [EXTRACTED]
- [[order_executor.py]] - `imports_from` [EXTRACTED]
- [[paper.py]] - `imports_from` [EXTRACTED]
- [[paths.py]] - `imports_from` [EXTRACTED]
- [[prob_threshold()]] - `contains` [EXTRACTED]
- [[schema_validator.py]] - `imports_from` [EXTRACTED]
- [[sql_normalize_iso_column()]] - `contains` [EXTRACTED]
- [[test_confidence_tiers.py]] - `calls` [EXTRACTED]
- [[test_cron_group_c.py]] - `references` [EXTRACTED]
- [[test_nws.py]] - `imports_from` [EXTRACTED]
- [[test_price_and_size.py]] - `imports` [EXTRACTED]
- [[test_trade_cycle_engine.py]] - `imports_from` [EXTRACTED]
- [[test_weather.py]] - `imports_from` [EXTRACTED]
- [[test_weather_markets.py]] - `imports_from` [EXTRACTED]
- [[test_web_app.py]] - `imports` [EXTRACTED]
- [[tracker.py]] - `imports_from` [EXTRACTED]
- [[trade_cycle.py]] - `imports_from` [EXTRACTED]
- [[trading_gates.py]] - `imports_from` [EXTRACTED]
- [[utc_today()]] - `contains` [EXTRACTED]
- [[weather_markets.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/NWS/Circuit-Breaker_Data_Validation