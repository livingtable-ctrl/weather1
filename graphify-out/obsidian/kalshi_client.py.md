---
source_file: "kalshi_client.py"
type: "code"
community: "Community 351"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_351
---

# kalshi_client.py

## Connections
- [[CircuitBreaker]] - `imports` [EXTRACTED]
- [[CircuitOpenError]] - `imports` [EXTRACTED]
- [[Grade Audit Module Doc kalshi_client.py]] - `references` [EXTRACTED]
- [[Grade Audit Module Doc order_executor.py]] - `references` [EXTRACTED]
- [[Kalshi API client with RSA-PSS authentication.]] - `rationale_for` [EXTRACTED]
- [[KalshiClient]] - `contains` [EXTRACTED]
- [[_build_session()]] - `contains` [EXTRACTED]
- [[_check_key_permissions()]] - `contains` [EXTRACTED]
- [[_check_key_permissions() Windows icacls Untested Gap (710)]] - `references` [EXTRACTED]
- [[_find_order_by_client_id() RF1 Bare Except (710)]] - `references` [EXTRACTED]
- [[_request_with_retry()]] - `contains` [EXTRACTED]
- [[_to_v2_side_price()]] - `contains` [EXTRACTED]
- [[circuit_breaker.py]] - `imports_from` [EXTRACTED]
- [[cron.py]] - `imports_from` [EXTRACTED]
- [[kalshi_client.py File Grade median 810, no active bugs]] - `references` [EXTRACTED]
- [[kalshi_client.py Grade Audit]] - `references` [EXTRACTED]
- [[log_api_request()]] - `imports` [EXTRACTED]
- [[output_formatters.py]] - `imports_from` [EXTRACTED]
- [[place_order() client_order_id Idempotency Design (810)]] - `references` [EXTRACTED]
- [[schema_validator.py]] - `imports_from` [EXTRACTED]
- [[test_infrastructure.py]] - `imports_from` [EXTRACTED]
- [[trade_cycle.py]] - `imports_from` [EXTRACTED]
- [[validate_market()]] - `imports` [EXTRACTED]
- [[weather_markets.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_351