---
source_file: "weather_markets.py"
type: "code"
community: "Ensemble Weight Blending Tests"
location: "L7228"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Ensemble_Weight_Blending_Tests
---

# _confidence_scaled_blend_weights()

## Connections
- [[31 _blend_weights scaled by inverse ensemble variance.]] - `rationale_for` [EXTRACTED]
- [[dot-_call()]] - `calls` [EXTRACTED]
- [[dot-test_high_ens_std_reduces_ensemble_weight()]] - `calls` [EXTRACTED]
- [[dot-test_low_ens_std_increases_ensemble_weight()]] - `calls` [EXTRACTED]
- [[dot-test_none_ens_std_returns_base_weights()]] - `calls` [EXTRACTED]
- [[dot-test_weights_sum_to_one()]] - `calls` [EXTRACTED]
- [[Phase 2 Batch K Regression Tests]] - `imports` [EXTRACTED]
- [[Phase 7 Data Safety Plan]] - `implements` [EXTRACTED]
- [[Weather Markets Grade Audit]] - `references` [EXTRACTED]
- [[_analyze_snow_trade()]] - `calls` [EXTRACTED]
- [[_blend_weights()]] - `calls` [EXTRACTED]
- [[analyze_trade()]] - `calls` [EXTRACTED]
- [[test_ensemble_confidence_scale_clamped()]] - `calls` [EXTRACTED]
- [[test_ensemble_confidence_scale_high_std_reduces_ens_weight()]] - `calls` [EXTRACTED]
- [[test_ensemble_confidence_scale_no_std_unchanged()]] - `calls` [EXTRACTED]
- [[test_forecasting.py]] - `imports` [EXTRACTED]
- [[test_weather_markets.py]] - `imports` [EXTRACTED]
- [[weather_markets.py_2]] - `implements` [EXTRACTED]
- [[weather_markets.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Ensemble_Weight_Blending_Tests