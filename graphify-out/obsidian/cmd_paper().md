---
source_file: "main.py"
type: "code"
community: "Community 1"
location: "L7819"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_1
---

# cmd_paper()

## Connections
- [[KalshiClient]] - `references` [EXTRACTED]
- [[KalshiClient_1]] - `references` [EXTRACTED]
- [[Paper trading commands paper buy ticker yesno qty price paper results…]] - `rationale_for` [EXTRACTED]
- [[Paper trading commands paper buy ticker yesno qty price paper results…_1]] - `rationale_for` [EXTRACTED]
- [[_ascii_chart()]] - `calls` [EXTRACTED]
- [[_header()]] - `calls` [EXTRACTED]
- [[_hourly_gates_active()]] - `calls` [EXTRACTED]
- [[_hurricane_count_gates_active()]] - `calls` [EXTRACTED]
- [[_hurricane_next_event_gates_active()]] - `calls` [EXTRACTED]
- [[_kv()]] - `calls` [EXTRACTED]
- [[_quick_paper_buy()]] - `calls` [EXTRACTED]
- [[_rain_gates_active()]] - `calls` [EXTRACTED]
- [[_snow_gates_active()]] - `calls` [EXTRACTED]
- [[_storm_order_gates_active()]] - `calls` [EXTRACTED]
- [[analyze_trade()]] - `calls` [EXTRACTED]
- [[bold()]] - `calls` [EXTRACTED]
- [[check_position_limits()]] - `calls` [EXTRACTED]
- [[cmd_menu()]] - `calls` [EXTRACTED]
- [[dim()]] - `calls` [EXTRACTED]
- [[enrich_with_forecast()]] - `calls` [EXTRACTED]
- [[get_all_trades()]] - `calls` [EXTRACTED]
- [[get_balance_history()]] - `calls` [EXTRACTED]
- [[get_expiry_date_clustering()]] - `calls` [EXTRACTED]
- [[get_factor_exposure()]] - `calls` [EXTRACTED]
- [[get_peak_balance()]] - `calls` [EXTRACTED]
- [[get_performance()]] - `calls` [EXTRACTED]
- [[get_unrealized_pnl_paper()]] - `calls` [EXTRACTED]
- [[graduation_check()]] - `calls` [EXTRACTED]
- [[green()]] - `calls` [EXTRACTED]
- [[is_hurricane_count_ticker()]] - `calls` [EXTRACTED]
- [[is_hurricane_next_event_ticker()]] - `calls` [EXTRACTED]
- [[is_hurricane_ticker()]] - `calls` [EXTRACTED]
- [[is_paused_drawdown()]] - `calls` [EXTRACTED]
- [[is_storm_order_ticker()]] - `calls` [EXTRACTED]
- [[is_trading_paused()]] - `calls` [EXTRACTED]
- [[kelly_bet_dollars()]] - `calls` [EXTRACTED]
- [[kelly_quantity()]] - `calls` [EXTRACTED]
- [[main()]] - `calls` [EXTRACTED]
- [[main.py]] - `contains` [EXTRACTED]
- [[paper.get_balance]] - `calls` [EXTRACTED]
- [[paper.get_open_trades]] - `calls` [EXTRACTED]
- [[place_paper_order()_1]] - `calls` [EXTRACTED]
- [[portfolio_kelly_fraction()]] - `calls` [EXTRACTED]
- [[red()]] - `calls` [EXTRACTED]
- [[reset_paper_account()]] - `calls` [EXTRACTED]
- [[settle_paper_trade()]] - `calls` [EXTRACTED]
- [[yellow()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_1