---
source_file: "safe_io.py"
type: "code"
community: "Black Swan Detection & Walk-Forward Backtest"
location: "L99"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Black_Swan_Detection__Walk-Forward_Backtest
---

# atomic_write_json()

## Connections
- [[dot-_save_cooldowns()]] - `calls` [EXTRACTED]
- [[dot-_save_history()]] - `calls` [EXTRACTED]
- [[dot-dump_to_disk()]] - `calls` [EXTRACTED]
- [[Grade Audit Preamble]] - `references` [EXTRACTED]
- [[Path_14]] - `references` [EXTRACTED]
- [[Phase 2 Infrastructure Plan]] - `implements` [EXTRACTED]
- [[Write data to path atomically (write temp → fsync → rename). Retries up to…]] - `rationale_for` [EXTRACTED]
- [[_atomic_write_payload()]] - `calls` [EXTRACTED]
- [[_cmd_cron_body()]] - `calls` [EXTRACTED]
- [[_notify_feature_activation()]] - `calls` [EXTRACTED]
- [[_save()]] - `references` [EXTRACTED]
- [[_save()_1]] - `calls` [EXTRACTED]
- [[_save_retired_strategies()]] - `calls` [EXTRACTED]
- [[_save_strategy_pins()]] - `calls` [EXTRACTED]
- [[_system_cooldown_elapsed()]] - `calls` [EXTRACTED]
- [[atomic_write_json_with_history()]] - `calls` [EXTRACTED]
- [[atomic_write_text()]] - `shares_data_with` [EXTRACTED]
- [[check_retirement_probation()]] - `calls` [EXTRACTED]
- [[check_series_drift()]] - `calls` [EXTRACTED]
- [[circuit_breaker.py]] - `imports` [EXTRACTED]
- [[cron.py]] - `imports` [EXTRACTED]
- [[fetch_archive_temps()]] - `calls` [EXTRACTED]
- [[fetch_historical()]] - `calls` [EXTRACTED]
- [[fetch_historical_daily()]] - `calls` [EXTRACTED]
- [[fetch_historical_daily_snow()]] - `calls` [EXTRACTED]
- [[fetch_pdo_pna()]] - `calls` [EXTRACTED]
- [[flush_ensemble_disk_cache()]] - `calls` [EXTRACTED]
- [[flush_forecast_disk_cache()]] - `calls` [EXTRACTED]
- [[forecast_cache.py]] - `imports` [EXTRACTED]
- [[hurricane_climatology.py HURDAT2 text cache]] - `references` [EXTRACTED]
- [[load_all_sigmas()]] - `calls` [EXTRACTED]
- [[log_city_registry_report()]] - `calls` [EXTRACTED]
- [[notify.py]] - `imports` [EXTRACTED]
- [[paper.py]] - `imports` [EXTRACTED]
- [[place_paper_order()_1]] - `calls` [EXTRACTED]
- [[refresh_hourly_target_hours()]] - `calls` [EXTRACTED]
- [[refresh_hurricane_count_to_date()]] - `calls` [EXTRACTED]
- [[safe_io.py_2]] - `implements` [EXTRACTED]
- [[safe_io.py]] - `contains` [EXTRACTED]
- [[safe_io.py_1]] - `implements` [EXTRACTED]
- [[save_alerts()]] - `calls` [EXTRACTED]
- [[save_correlations()]] - `calls` [EXTRACTED]
- [[save_learned_weights()]] - `calls` [EXTRACTED]
- [[save_walk_forward_params()]] - `calls` [EXTRACTED]
- [[settle_paper_trade()]] - `calls` [EXTRACTED]
- [[test_atomic_write_creates_file()]] - `calls` [EXTRACTED]
- [[test_atomic_write_falls_back_to_tmp_on_oserror()]] - `calls` [EXTRACTED]
- [[test_atomic_write_is_atomic()]] - `calls` [EXTRACTED]
- [[test_atomic_write_raises_on_double_failure()]] - `calls` [EXTRACTED]
- [[test_infrastructure.py]] - `references` [EXTRACTED]
- [[test_notify.py]] - `references` [EXTRACTED]
- [[test_p9_p10.py]] - `references` [EXTRACTED]
- [[test_safe_io.py]] - `calls` [EXTRACTED]
- [[tracker predictions table]] - `references` [EXTRACTED]
- [[write_settlement_signals()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Black_Swan_Detection__Walk-Forward_Backtest