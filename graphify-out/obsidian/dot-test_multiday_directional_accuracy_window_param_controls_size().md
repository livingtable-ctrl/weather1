---
source_file: "tests/test_backlog_batch.py"
type: "code"
community: "Kelly City Multiplier & Edge Realization"
location: "L528"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Kelly_City_Multiplier__Edge_Realization
---

# .test_multiday_directional_accuracy_window_param_controls_size()

## Connections
- [[dot-_make_multiday_trade()]] - `calls` [EXTRACTED]
- [[A smaller window should be able to see a different (worse) recent streak than a…]] - `rationale_for` [EXTRACTED]
- [[TestEdgeRealizationRate]] - `method` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Kelly_City_Multiplier__Edge_Realization