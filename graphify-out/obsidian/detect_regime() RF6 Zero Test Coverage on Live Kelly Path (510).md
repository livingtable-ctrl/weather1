---
source_file: "docs/grade_audit/outputs/regime.py.md"
type: "document"
community: "Community 445"
location: "L:10-98"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_445
---

# detect_regime() RF6: Zero Test Coverage on Live Kelly Path (5/10)

## Connections
- [[I5 Kelly FiniteRange Guard]] - `references` [EXTRACTED]
- [[RF6 No Test Coverage on Trade Path]] - `references` [EXTRACTED]
- [[apply_ml_prob_correction() RF1 DEBUG on Model Failure (610)]] - `semantically_similar_to` [INFERRED]
- [[drawdown_scaling_factor() Tiered Kelly Scaling (910)]] - `semantically_similar_to` [INFERRED]
- [[regime.py]] - `references` [EXTRACTED]
- [[weather_markets.py]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_445