---
source_file: "kalshi_ws.py"
type: "code"
community: "Community 227"
location: "L138"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_227
---

# update_orderbook_cache()

## Connections
- [[dot-test_delta_message_does_not_feed_flash_crash_breaker()]] - `calls` [EXTRACTED]
- [[dot-test_orderbook_delta_does_not_refresh_mid_price_timestamp()]] - `calls` [EXTRACTED]
- [[dot-test_ticker_message_feeds_flash_crash_breaker()]] - `calls` [EXTRACTED]
- [[dot-test_update_and_read_cache()]] - `calls` [EXTRACTED]
- [[FlashCrashCB]] - `calls` [EXTRACTED]
- [[Update in-memory and on-disk cache for a ticker.]] - `rationale_for` [EXTRACTED]
- [[_get_current_book()]] - `calls` [EXTRACTED]
- [[_ws_listener()]] - `calls` [EXTRACTED]
- [[flash_crash_cb (FlashCrashCB instance)]] - `calls` [EXTRACTED]
- [[kalshi_ws.py]] - `contains` [EXTRACTED]
- [[order_executor.py_1]] - `shares_data_with` [EXTRACTED]
- [[test_kalshi_ws.py]] - `references` [EXTRACTED]
- [[test_live_execution.py]] - `references` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_227