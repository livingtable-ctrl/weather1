---
source_file: "order_executor.py"
type: "code"
community: "Community 110"
location: "L1182"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_110
---

# _exit_live_position()

## Connections
- [[dot-exit()]] - `calls` [EXTRACTED]
- [[dot-test_full_fill_exit_order_not_treated_as_new_open_position()]] - `calls` [EXTRACTED]
- [[dot-test_full_fill_records_fee_adjusted_pnl()]] - `calls` [EXTRACTED]
- [[dot-test_gain_case_applies_fee_discount()]] - `calls` [EXTRACTED]
- [[dot-test_gate_blocked_returns_false_and_places_nothing()_1]] - `calls` [EXTRACTED]
- [[dot-test_ioc_no_fill_leaves_position_open()]] - `calls` [EXTRACTED]
- [[dot-test_no_side_exit_pnl_uses_no_side_prices_directly()]] - `calls` [EXTRACTED]
- [[dot-test_partial_fill_gain_case_applies_fee_discount()]] - `calls` [EXTRACTED]
- [[dot-test_partial_fill_reconciles_quantity_and_realizes_pnl()]] - `calls` [EXTRACTED]
- [[dot-test_place_order_exception_logs_failed_status()]] - `calls` [EXTRACTED]
- [[dot-test_reflects_reduced_quantity_after_partial_exit()]] - `calls` [EXTRACTED]
- [[Place an immediate taker-cross sell to close an open live position. Re-runs…]] - `rationale_for` [EXTRACTED]
- [[_check_live_model_exits()]] - `calls` [EXTRACTED]
- [[_to_fill_count()]] - `calls` [EXTRACTED]
- [[add_live_loss()]] - `calls` [EXTRACTED]
- [[get_filled_unsettled_live_orders()]] - `calls` [EXTRACTED]
- [[log_order()]] - `calls` [EXTRACTED]
- [[log_order_result()]] - `calls` [EXTRACTED]
- [[order_executor.py]] - `contains` [EXTRACTED]
- [[pre_live_trade_check()]] - `calls` [EXTRACTED]
- [[record_live_early_exit()]] - `calls` [EXTRACTED]
- [[record_live_partial_exit()]] - `calls` [EXTRACTED]
- [[test_live_execution.py]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_110