---
source_file: "tests/test_live_execution.py"
type: "code"
community: "Community 338"
location: "L441"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_338
---

# TestRecoverPendingOrders

## Connections
- [[dot-setup_method()_12]] - `method` [EXTRACTED]
- [[dot-teardown_method()_11]] - `method` [EXTRACTED]
- [[dot-test_canceled_order_resolves_to_canceled()]] - `method` [EXTRACTED]
- [[dot-test_executed_order_resolves_to_internal_filled_status()]] - `method` [EXTRACTED]
- [[dot-test_partial_fill_then_cancel_resolves_to_filled()]] - `method` [EXTRACTED]
- [[dot-test_resting_order_recovery_preserves_response_for_fill_polling()]] - `method` [EXTRACTED]
- [[dot-test_resting_order_resolves_to_pending()]] - `method` [EXTRACTED]
- [[2026-07-09 Kalshi's real order-status enum is restingcanceledexecuted --…]] - `rationale_for` [EXTRACTED]
- [[LivePositionStore]] - `uses` [INFERRED]
- [[Position]] - `uses` [INFERRED]
- [[test_live_execution.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_338