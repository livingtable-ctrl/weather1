---
source_file: "paper.py"
type: "code"
community: "Community 189"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_189
---

# paper.position_correlation_matrix

## Connections
- [[dot-test_diagonal_is_one()]] - `calls` [EXTRACTED]
- [[dot-test_empty_returns_empty()]] - `calls` [EXTRACTED]
- [[dot-test_known_city_pair_uses_lookup()]] - `calls` [EXTRACTED]
- [[dot-test_matrix_is_symmetric()]] - `calls` [EXTRACTED]
- [[dot-test_same_city_adjacent_date_is_0_50()]] - `calls` [EXTRACTED]
- [[dot-test_same_city_distant_dates_is_0_30()]] - `calls` [EXTRACTED]
- [[dot-test_same_city_same_date_is_0_85()]] - `calls` [EXTRACTED]
- [[dot-test_unknown_city_pair_defaults_to_0_10()]] - `calls` [EXTRACTED]
- [[Build NxN correlation matrix for a list of trades. Correlation rules Same city…]] - `rationale_for` [EXTRACTED]
- [[TestPositionCorrelationMatrix]] - `references` [EXTRACTED]
- [[monte_carlo.py]] - `imports` [EXTRACTED]
- [[paper.corr_kelly_scale]] - `calls` [EXTRACTED]
- [[paper.py]] - `contains` [EXTRACTED]
- [[simulate_portfolio()]] - `calls` [EXTRACTED]
- [[test_paper.py]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_189