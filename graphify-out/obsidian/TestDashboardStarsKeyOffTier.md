---
source_file: "tests/test_trade_cycle_engine.py"
type: "code"
community: "Community 134"
location: "L1367"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_134
---

# TestDashboardStarsKeyOffTier

## Connections
- [[dot-test_real_med_tier_shows_two_stars()]] - `method` [EXTRACTED]
- [[dot-test_real_strong_tier_at_low_time_risk_still_shows_three_stars()]] - `method` [EXTRACTED]
- [[dot-test_real_strong_tier_at_non_low_time_risk_shows_two_stars()]] - `method` [EXTRACTED]
- [[dot-test_signals_cache_entry_carries_tier_for_downstream_summaries()]] - `method` [EXTRACTED]
- [[dot-test_untiered_strong_text_no_longer_shows_multiple_stars()]] - `method` [EXTRACTED]
- [[LiveTradingGate]] - `uses` [INFERRED]
- [[Violation]] - `uses` [INFERRED]
- [[backlog.txt 'DASHBOARD STARS + WATCH-MODE STRONG ALERT KEY OFF SIGNAL TEXT, NOT…]] - `rationale_for` [EXTRACTED]
- [[test_trade_cycle_engine.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_134