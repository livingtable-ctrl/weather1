---
source_file: "COMMANDS.md"
type: "document"
community: "Community 693"
location: "L1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_693
---

# Kalshi Weather Bot — Command Reference

## Connections
- [[Admin (use with care)]] - `contains` [EXTRACTED]
- [[Analysis & Calibration]] - `contains` [EXTRACTED]
- [[COMMANDS]] - `contains` [EXTRACTED]
- [[Daily Use]] - `contains` [EXTRACTED]
- [[Reporting]] - `contains` [EXTRACTED]
- [[Setup & Config]] - `contains` [EXTRACTED]
- [[Subcommand Notes]] - `contains` [EXTRACTED]
- [[System & Safety]] - `contains` [EXTRACTED]
- [[Trading]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_693