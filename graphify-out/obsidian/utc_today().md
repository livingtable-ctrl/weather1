---
source_file: "utils.py"
type: "code"
community: "Climatology & Climate Index Fetching"
location: "L16"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Climatology__Climate_Index_Fetching
---

# utc_today()

## Connections
- [[dot-test_cache_refreshes_after_ttl()]] - `calls` [EXTRACTED]
- [[dot-test_cache_served_within_ttl()]] - `calls` [EXTRACTED]
- [[dot-test_count_settled_signal_rows_multiday_excludes_sameday_rows()]] - `calls` [EXTRACTED]
- [[dot-test_matches_datetime_now_utc()]] - `calls` [EXTRACTED]
- [[dot-test_only_daily_rows_reach_the_fit_call()]] - `calls` [EXTRACTED]
- [[dot-test_only_daily_rows_reach_the_fit_call_snow()]] - `calls` [EXTRACTED]
- [[dot-test_past_date_only_portfolio_returns_empty_result()]] - `calls` [EXTRACTED]
- [[dot-test_past_date_trade_excluded_from_simulation()]] - `calls` [EXTRACTED]
- [[dot-test_returns_date_object()]] - `calls` [EXTRACTED]
- [[dot-test_snow_rows_excluded_from_sameday_pool()]] - `calls` [EXTRACTED]
- [[Phase 2 Batch C Regression Tests]] - `imports` [EXTRACTED]
- [[Phase 2 Batch H Regression Tests]] - `imports` [EXTRACTED]
- [[Return the current UTC date. Use everywhere instead of date.today().]] - `rationale_for` [EXTRACTED]
- [[_build_app()]] - `calls` [EXTRACTED]
- [[_cmd_cron_body()]] - `calls` [EXTRACTED]
- [[_feature_importance_days_out()]] - `calls` [EXTRACTED]
- [[_fetch_previous_run_daily()]] - `calls` [EXTRACTED]
- [[_fetch_previous_run_leads()]] - `calls` [EXTRACTED]
- [[_local_or_utc_today()]] - `calls` [EXTRACTED]
- [[acis_precip.py]] - `imports` [EXTRACTED]
- [[acis_snow.py]] - `imports` [EXTRACTED]
- [[auto_backup()]] - `calls` [EXTRACTED]
- [[backtest.py]] - `imports` [EXTRACTED]
- [[climate_indices.py]] - `imports` [EXTRACTED]
- [[climatology.py]] - `imports` [EXTRACTED]
- [[cmd_forecast()]] - `calls` [EXTRACTED]
- [[cmd_menu()]] - `calls` [EXTRACTED]
- [[cmd_watch_settle()]] - `calls` [EXTRACTED]
- [[cron.py]] - `imports` [EXTRACTED]
- [[date_10]] - `references` [EXTRACTED]
- [[fetch_historical()]] - `calls` [EXTRACTED]
- [[fetch_historical_daily()]] - `calls` [EXTRACTED]
- [[fetch_historical_daily_snow()]] - `calls` [EXTRACTED]
- [[get_indices()]] - `calls` [EXTRACTED]
- [[log_prediction()]] - `calls` [INFERRED]
- [[main.py]] - `imports` [EXTRACTED]
- [[monte_carlo.py]] - `imports` [EXTRACTED]
- [[mos.py]] - `imports` [EXTRACTED]
- [[nws.py]] - `imports` [EXTRACTED]
- [[nws_prob()]] - `calls` [INFERRED]
- [[run_backtest()]] - `calls` [EXTRACTED]
- [[run_walk_forward()]] - `calls` [EXTRACTED]
- [[simulate_portfolio()]] - `calls` [INFERRED]
- [[test_date_today_guard.py]] - `references` [EXTRACTED]
- [[test_feature_importance_days_out.py]] - `calls` [EXTRACTED]
- [[test_ml_bias.py]] - `imports` [EXTRACTED]
- [[test_paper.py]] - `imports` [EXTRACTED]
- [[test_tracker.py]] - `imports` [EXTRACTED]
- [[tracker.py]] - `imports` [EXTRACTED]
- [[utils.py]] - `contains` [EXTRACTED]
- [[web_app.py]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Climatology__Climate_Index_Fetching