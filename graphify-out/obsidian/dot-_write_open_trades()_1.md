---
source_file: "tests/test_paper.py"
type: "code"
community: "Community 138"
location: "L2635"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_138
---

# ._write_open_trades()

## Connections
- [[dot-test_client_none_returns_zero_even_with_open_trades()]] - `calls` [EXTRACTED]
- [[dot-test_exact_040_boundary_is_balanced()]] - `calls` [EXTRACTED]
- [[dot-test_exact_060_boundary_is_balanced()]] - `calls` [EXTRACTED]
- [[dot-test_mixed_yes_and_no_sides_sum_correctly()]] - `calls` [EXTRACTED]
- [[dot-test_no_heavy_below_040()]] - `calls` [EXTRACTED]
- [[dot-test_no_open_trades_is_balanced_with_zero_costs()]] - `calls` [EXTRACTED]
- [[dot-test_no_open_trades_returns_zero()]] - `calls` [EXTRACTED]
- [[dot-test_no_side_marks_at_one_minus_ask()]] - `calls` [EXTRACTED]
- [[dot-test_yes_heavy_above_060()]] - `calls` [EXTRACTED]
- [[dot-test_yes_side_marks_at_bid()]] - `calls` [EXTRACTED]
- [[TestGetUnrealizedPnlPaper]] - `method` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_138